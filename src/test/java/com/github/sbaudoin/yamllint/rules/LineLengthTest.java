...
class LineLengthTest extends RuleTester {
  ...
  void nonBreakableWord() throws Exception {
    YamlLintConfig conf =
        getConfig(
            "line-length: {max: 20, allow-non-breakable-words: true, allow-non-breakable-inline-mappings: false}");
    check("---\n" + Format.repeat(30, "A") + "\n", conf);
    check(
        """
              ---
    ...
    check(
        """
              ...
              """,
        conf,
        getLintProblem(2, 2, "comments"),
        getLintProblem(2, 21, "line-length"));

    conf =
        getConfig(
            "line-length: {max: 20, allow-non-breakable-words: false, allow-non-breakable-inline-mappings: false}");
  ...
  void withDosNewlines() throws Exception {
    YamlLintConfig conf =
        getConfig(
            "line-length: {max: 10}", "new-lines: {type: dos}", "new-line-at-end-of-file: disable");
    check("---\r\nABCD EFGHI", conf);
    check("---\r\nABCD EFGHI\r\n", conf);
    check("---\r\nABCD EFGHIJ", conf, getLintProblem(2, 11));
    check("---\r\nABCD EFGHIJ\r\n", conf, getLintProblem(2, 11));
