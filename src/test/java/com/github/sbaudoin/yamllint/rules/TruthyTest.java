...
class TruthyTest extends RuleTester {
  ...
  void differentAllowedValues() throws Exception {
    YamlLintConfig conf = getConfig("truthy:", "  allowed-values: [\"yes\", \"no\"]");
    check(
        """
              ---
              key1: foo
  ...
  void combinedAllowedValues() throws Exception {
    YamlLintConfig conf =
        getConfig("truthy:", "  allowed-values: [\"yes\", \"no\", \"true\", \"false\"]");
    check(
        """
              ---
              key1: foo
  ...
  void noAllowedValues() throws Exception {
    YamlLintConfig conf = getConfig("truthy:", "  allowed-values: []");
    check(
        """
              ---
              key1: foo
  ...
  void checkKeysDisabled() throws Exception {
    YamlLintConfig conf =
        getConfig(
            "truthy:", "  allowed-values: []", "  check-keys: false", "key-duplicates: disable");
    check(
        """
              ---
              YES: 0
