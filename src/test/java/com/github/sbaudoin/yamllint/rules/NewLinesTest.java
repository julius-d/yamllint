...
class NewLinesTest extends RuleTester {
  ...
  void disabled() throws Exception {
    YamlLintConfig conf = getConfig("new-line-at-end-of-file: disable", "new-lines: disable");
    check("", conf);
    check("\n", conf);
    check("\r", conf);
    check("\r\n", conf);
  ...
  void unixType() throws Exception {
    YamlLintConfig conf = getConfig("new-line-at-end-of-file: disable", "new-lines: {type: unix}");
    check("", conf);
    check("\r", conf);
    check("\n", conf);
  ...
  void dosType() throws Exception {
    YamlLintConfig conf = getConfig("new-line-at-end-of-file: disable", "new-lines: {type: dos}");
    check("", conf);
    check("\r", conf);
    check("\n", conf, getLintProblem(1, 1));
    check("\r\n", conf);
  ...
  void platformType() throws Exception {
    YamlLintConfig conf =
        getConfig("new-line-at-end-of-file: disable", "new-lines: {type: platform}");
