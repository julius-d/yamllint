...
class EmptyLinesTest extends RuleTester {
  ...
  void disabled() throws Exception {
    YamlLintConfig conf =
        getConfig(
            "empty-lines: disable", "new-line-at-end-of-file: disable", "document-start: disable");
    check("", conf);
    // Unix
    check("\n", conf);
    check("\n\n", conf);
  ...
  void test0EmptyLines() throws Exception {
    YamlLintConfig conf =
        getConfig(
            "empty-lines: {max: 0, max-start: 0, max-end: 0}", "new-line-at-end-of-file: disable");
    // Unix
    check("---\n", conf);
  ...
  void spaces() throws Exception {
    YamlLintConfig conf =
        getConfig("empty-lines: {max: 1, max-start: 0, max-end: 0}", "trailing-spaces: disable");
    // Unix
    check("---\nintro\n\n \n\nconclusion\n", conf);
  ...
  void emptyLinesAtStart() throws Exception {
    YamlLintConfig conf =
        getConfig("empty-lines: {max: 2, max-start: 4, max-end: 0}", "document-start: disable");
    // Unix
    check("\n\n\n\nnon empty\n", conf);
    ...
    conf = getConfig("empty-lines: {max: 2, max-start: 0, max-end: 0}", "document-start: disable");
    // Unix
    check("non empty\n", conf);
  ...
  void emptyLinesAtEnd() throws Exception {
    YamlLintConfig conf =
        getConfig("empty-lines: {max: 2, max-start: 0, max-end: 4}", "document-start: disable");
    // Unix
    check("non empty\n\n\n\n\n", conf);
    check("non empty\n\n\n\n\n\n", conf, getLintProblem(6, 1));
    // Windows
