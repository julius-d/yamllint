...
public class Format {
  ...
  /**
   * ANSI code used to reset text decoration. Any other {@code ANSI_} code must be followed by this
   * code once the text has been decorated in order to come back to the standard text output format.
   */
  ...
  public static String parsable(LintProblem problem, String filename) {
    return "%1$s:%2$d:%3$d:%4$s:%5$s:%6$s"
        .formatted(
            filename,
            problem.getLine(),
            problem.getColumn(),
  ...
  public static String github(LintProblem problem, String filename) {
    return "::%5$s file=%1$s,line=%2$d,col=%3$s::%4$s%6$s"
        .formatted(
            filename,
            problem.getLine(),
            problem.getColumn(),
  ...
  /**
   * Resolves the passed format, i.e. tells which is the actual, non-auto format to use as the
   * output format
   *
