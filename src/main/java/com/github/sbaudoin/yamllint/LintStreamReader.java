...
/**
 * <code>StreamReader</code> intended to replace SnakeYAML's standard class for loading the whole
 * content in memory. This is not optimized but this is a requirement to permanently be able to have
 * a complete access to the complete file content without windowing.
 */
public class LintStreamReader extends StreamReader {
  ...
  public void forward(int length) {
    for (int i = 0; i < length && hasEnoughData(); i++) {
      ...
      this.index++;
      if (Constant.LINEBR.has(c) || (c == '\r' && (hasEnoughData() && buffer[pointer] != '\n'))) {
        this.line++;
        this.column = 0;
      } else if (c != 0xFEFF) {
        this.column++;
  ...
  public String prefix(int length) {
    if (length == 0) {
      ...
      return new String(this.buffer, pointer, Math.min(length, dataLength - pointer));
    }
  }
