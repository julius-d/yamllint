...
public class Parser {
  ...
  /**
   * Simple wrapper class for lines: a line is contained within a given string (the boundaries are
   * given by <var>start</var> and <var>end</var>) and has a line number
   */
  ...
  public static class Token implements Lined {
    ...
    public Token(
        int lineNo,
        org.yaml.snakeyaml.tokens.Token curr,
        org.yaml.snakeyaml.tokens.Token prev,
        org.yaml.snakeyaml.tokens.Token next,
        org.yaml.snakeyaml.tokens.Token nextnext) {
      this.lineNo = lineNo;
      this.curr = curr;
      this.prev = prev;
      this.next = next;
  ...
  /**
   * Simple wrapper class for comments: a YAML comment start with a {@code #} character, is located
   * on a line and starts at a given column number on this line. It also has tokens located before
   * and after it.
   */
  public static class Comment implements Lined {
    ...
    public Comment(
        ...
        org.yaml.snakeyaml.tokens.Token tokenAfter,
    ...
    /**
     * Returns {@code true} if the comment is inline. Inline comments appear on a line after some
     * text:
     *
     * <pre>
   ...
   */
  public static List<Comment> commentsBetweenTokens(
      final org.yaml.snakeyaml.tokens.Token token1, final org.yaml.snakeyaml.tokens.Token token2) {
    ...
    if (token2 == null) {
      buf =
          toString(
              Arrays.copyOfRange(
                  token1.getEndMark().getBuffer(),
                  token1.getEndMark().getPointer(),
                  token1.getEndMark().getBuffer().length));
      ...
      buf =
          toString(
              Arrays.copyOfRange(
                  token1.getEndMark().getBuffer(),
                  token1.getEndMark().getPointer(),
                  token2.getStartMark().getPointer()));
    }
      ...
      if (pos != -1) {
        Comment comment =
            new Comment(
                lineNo,
                columnNo + pos,
                toString(token1.getEndMark().getBuffer()),
                pointer + pos,
                token1,
                token2,
                commentBefore);
  ...
  public static List<Lined> getTokensOrComments(final String buffer) {
    ...
    try {
      ...
      while (curr != null) {
        ...
        tokensOrComments.add(
            new Token(curr.getStartMark().getLine() + 1, curr, prev, next, nextnext));
