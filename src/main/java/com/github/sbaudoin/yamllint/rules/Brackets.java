 ...
 *
 * <ul>
 *   <li>{@code forbid} is used to forbid the use of flow sequences which are denoted by surrounding
 *       brackets (<code>[</code> and <code>]</code>). Use {@code true} to forbid the use of flow
 *       sequences completely. Use {@code non-empty} to forbid the use of all flow sequences except
 *       for empty ones.
 *   <li>{@code min-spaces-inside} defines the minimal number of spaces required inside brackets.
 *   <li>{@code max-spaces-inside} defines the maximal number of spaces allowed inside brackets.
 *   <li>{@code min-spaces-inside-empty} defines the minimal number of spaces required inside empty
 *       brackets.
 *   <li>{@code max-spaces-inside-empty} defines the maximal number of spaces allowed inside empty
 *       brackets.
 * </ul>
 *
 * <p>Examples:
 ...
 * </pre>
 *
 * <p>With <code>brackets: {min-spaces-inside: 0, max-spaces-inside: 0}</code> the following code
 * snippet would **PASS**:
 *
 * <pre>object: [1, 2, abc]</pre>
 *
 ...
 * <pre>object: [ 1, 2, abc ]</pre>
 *
 * <p>With <code>brackets: {min-spaces-inside: 1, max-spaces-inside: 3}</code> the following code
 * snippet would **PASS**:
 *
 * <pre>object: [ 1, 2, abc ]</pre>
 *
 ...
 * <pre>object: [1, 2, abc ]</pre>
 *
 * <p>With <code>brackets: {min-spaces-inside-empty: 0, max-spaces-inside-empty: 0}</code> the
 ...
 * <pre>object: [ ]</pre>
 *
 * <p>With <code>brackets: {min-spaces-inside-empty: 1, max-spaces-inside-empty: -1}</code> the
 * following code snippet would **PASS**:
 *
 * <pre>object: [         ]</pre>
 *
...
public class Brackets extends TokenRule {
  ...
  public List<LintProblem> check(
    ...
    if ((Boolean.TRUE.equals(conf.get(OPTION_FORBID)) && token instanceof FlowSequenceStartToken)
        || ("non-empty".equals(conf.get(OPTION_FORBID))
            && token instanceof FlowSequenceStartToken
    ...
    if (token instanceof FlowSequenceStartToken && next instanceof FlowSequenceEndToken) {
      problem =
          spacesAfter(
              ...
              next,
              (int)
                  ((conf.get(OPTION_MIN_SPACES_INSIDE_EMPTY).equals(-1))
                      ? conf.get(OPTION_MIN_SPACES_INSIDE)
                      : conf.get(OPTION_MIN_SPACES_INSIDE_EMPTY)),
              (int)
                  ((conf.get(OPTION_MAX_SPACES_INSIDE_EMPTY).equals(-1))
                      ? conf.get(OPTION_MAX_SPACES_INSIDE)
                      : conf.get(OPTION_MAX_SPACES_INSIDE_EMPTY)),
