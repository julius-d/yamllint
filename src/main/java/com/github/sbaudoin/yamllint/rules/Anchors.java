 ...
 *
 * <ul>
 *   <li>Set {@code forbid-undeclared-aliases} to {@code true} to avoid aliases that reference an
 *       anchor that hasn't been declared (either not declared at all, or declared later in the
 *       document).
 *   <li>Set {@code forbid-duplicated-anchors} to {@code true} to avoid duplications of a same
 *       anchor.
 *   <li>Set {@code forbid-unused-anchors} to {@code true} to avoid anchors being declared but not
 *       used anywhere in the YAML document via alias.
 * </ul>
 *
 ...
 * <p>Examples:
 *
 * <p>With <code>anchors: {forbid-undeclared-aliases: true}</code> the following code snippet would
 * **PASS**:
 *
 * <pre>
 *     ---
 ...
 * </pre>
 *
 * <p>With <code>anchors: {forbid-duplicated-anchors: true}</code> the following code snippet would
 * **PASS**:
 *
 * <pre>
 *     ---
 ...
 * </pre>
 *
 * <p>With <code>anchors: {forbid-unused-anchors: true}</code> the following code snippet would
 * **PASS**:
 *
 * <pre>
 *     ---
...
public class Anchors extends TokenRule {
  ...
  public List<LintProblem> check(
    ...
    if (Boolean.TRUE.equals(conf.get(OPTION_FORBID_UNDECLARED_ALIASES))
        && token instanceof AliasToken aliasToken
    ...
    if (Boolean.TRUE.equals(conf.get(OPTION_FORBID_DUPLICATED_ANCHORS))
        && token instanceof AnchorToken anchorToken
      ...
      if (next instanceof StreamEndToken
        ...
        ((HashMap<String, Object>)
                (getContextMap(context)
                    .getOrDefault(aliasToken.getValue(), new HashMap<String, Object>())))
            .put("used", true);
      }
    }
