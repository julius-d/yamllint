 ...
 *
 * <ul>
 *   <li>{@code max} defines the maximal number of consecutive empty lines allowed in the document.
 *   <li>{@code max-start} defines the maximal number of empty lines allowed at the beginning of the
 *       file. This option takes precedence over {@code max}.
 *   <li>{@code max-end} defines the maximal number of empty lines allowed at the end of the file.
 *       This option takes precedence over {@code max}.
 * </ul>
 *
 * <p>Examples:
 *
 * <p>With <code>empty-lines: {max: 1}</code> the following code snippet would **PASS**:
 *
 * <pre>
 *     - foo:
 *         - 1
  ...
  public List<LintProblem> check(Map<Object, Object> conf, Parser.Line line) {
    ...
    if (line.getStart() == line.getEnd() && line.getEnd() < line.getBuffer().length()) {
      ...
      if (blankLines > max) {
        problems.add(
            new LintProblem(
                line.getLineNo(), 1, "too many blank lines (" + blankLines + " > " + max + ")"));
      }
    }
