...
/**
 * Use this rule to prevent values with octal numbers. In YAML, numbers that start with {@code 0}
 * are interpreted as octal, but this is not always wanted. For instance {@code 010} is the city
 * code of Beijing, and should not be converted to {@code 8}.
 *
 * <p>Examples:
 *
 * <p>With <code>octal-values: {forbid-implicit-octal: true}</code> the following code snippets
 * would **PASS**:
 *
 * <pre>
 *     user:
 ...
 * </pre>
 *
 * <p>With <code>octal-values: {forbid-explicit-octal: true}</code> the following code snippet would
 * **PASS**:
 *
 * <pre>
 *     user:
      ...
      if (token instanceof ScalarToken scalarToken) {
        if (scalarToken.getStyle().getChar() == null) {
          ...
          if (isDigit(val)
              && val.length() > 1
              && val.charAt(0) == '0'
              && val.substring(1).matches(IS_OCTAL_NUMBER_PATTERN)) {
            problems.add(
                new LintProblem(
    ...
    if ((boolean) conf.get(OPTION_FORBID_EXPLICIT_OCTAL)) {
      if (token instanceof ScalarToken scalarToken) {
        if (scalarToken.getStyle().getChar() == null) {
          String val = scalarToken.getValue();
          if (val.length() > 2
              && "0o".equals(val.substring(0, 2))
              && val.substring(2).matches(IS_OCTAL_NUMBER_PATTERN)) {
            problems.add(
                new LintProblem(
                    token.getStartMark().getLine() + 1,
                    token.getEndMark().getColumn() + 1,
