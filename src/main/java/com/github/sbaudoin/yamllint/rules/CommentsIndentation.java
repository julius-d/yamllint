 ...
 * <p>Examples:
 *
 * <p>With <code>comments-indentation: {}</code> the following code snippet would **PASS**:
 *
 * <pre>
 *     # Fibonacci
 *     [0, 1, 1, 2, 3, 5]
...
public class CommentsIndentation extends CommentRule {
  ...
  public List<LintProblem> check(Map<Object, Object> conf, Parser.Comment comment) {
    ...
    if (!(comment.getTokenBefore() instanceof StreamStartToken)
        && comment.getTokenBefore().getEndMark().getLine() + 1 == comment.getLineNo()) {
      return problems;
    }
    ...
    if (comment.getColumnNo() - 1 != prevLineIndent
        && comment.getColumnNo() - 1 != nextLineIndent) {
      problems.add(
          new LintProblem(
              comment.getLineNo(), comment.getColumnNo(), "comment not indented like content"));
    }
