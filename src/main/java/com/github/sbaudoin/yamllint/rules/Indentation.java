 ...
 *
 * <ul>
 *   <li>{@code spaces} defines the indentation width, in spaces. Set either to an integer (e.g.
 *       {@code 2} or {@code 4}, representing the number of spaces in an indentation level) or to
 *       {@code consistent} to allow any number, as long as it remains the same within the file.
 *   <li>{@code indent-sequences} defines whether block sequences should be indented or not (when in
 *       a mapping, this indentation is not mandatory -- some people perceive the {@code -} as part
 *       of the indentation). Possible values: {@code true}, {@code false}, {@code whatever} and
 *       {@code consistent}. {@code consistent} requires either all block sequences to be indented,
 *       or none to be. {@code whatever} means either indenting or not indenting individual block
 *       sequences is OK.
 *   <li>{@code check-multi-line-strings} defines whether to lint indentation in multi-line strings.
 *       Set to {@code true} to enable, {@code false} to disable.
 * </ul>
 *
 * <p>Examples:
 *
 * <p>With <code>indentation: {spaces: 1}</code> the following code snippet would **PASS**:
 *
 * <pre>
 *     history:
 *      - name: Unix
 ...
 * </pre>
 *
 * <p>With <code>indentation: {spaces: 4}</code> the following code snippet would **PASS**:
 *
 * <pre>
 *     history:
 *         - name: Unix
 ...
 * </pre>
 *
 * <p>With <code>indentation: {spaces: consistent}</code> the following code snippet would **PASS**:
 *
 * <pre>
 *     history:
 *        - name: Unix
 ...
 * </pre>
 *
 * <p>With <code>indentation: {spaces: 2, indent-sequences: false}</code> the following code snippet
 * would **PASS**:
 *
 * <pre>
 *     list:
 ...
 * </pre>
 *
 * <p>With <code>indentation: {spaces: 2, indent-sequences: whatever}</code> the following code
 * snippet would **PASS**:
 *
 * <pre>
 *     list:
 ...
 * </pre>
 *
 * <p>With <code>indentation: {spaces: 2, indent-sequences: consistent}</code> the following code
 * snippet would **PASS**:
 *
 * <pre>
 *     - flying:
 ...
 * </pre>
 *
 * <p>With <code>indentation: {spaces: 4, check-multi-line-strings: true}</code> the following code
 * snippet would **PASS**:
 *
 * <pre>
 *     Blaise Pascal:
...
public class Indentation extends TokenRule {
  ...
  public Indentation() {
    ...
    registerOption(
        OPTION_INDENT_SEQUENCES, Arrays.asList(Boolean.class, "whatever", OPTION_CONSISTENT), true);
  ...
  private int computeExpectedIndent(int foundIndent, Token token, Map<String, Object> context) {
    ...
    DumperOptions.ScalarStyle style =
        (DumperOptions.ScalarStyle) invokeSimpleMethod(token, "getStyle");
  ...
  private List<LintProblem> checkScalarIndentation(Token token, Map<String, Object> context) {
    ...
    while (true) {
      lineStart =
          find(
                  token.getStartMark().getBuffer(),
                  '\n',
                  lineStart,
                  token.getEndMark().getPointer() - 1)
              + 1;
      if (lineStart == 0) {
        break;
      }
  ...
  private List<LintProblem> checkToken(
      Map<Object, Object> conf,
      Token token,
      Token prev,
      Token next,
    ...
    boolean isVisible =
        (!(token instanceof StreamStartToken || token instanceof StreamEndToken))
            && !(token instanceof BlockEndToken)
            && !(token instanceof ScalarToken && "".equals(invokeSimpleMethod(token, "getValue")));
    boolean firstInLine =
        (isVisible && token.getStartMark().getLine() + 1 > (int) context.get(CURRENT_LINE_KEY));
    ...
    if (firstInLine) {
      ...
      if (!foundIndentation.equals(expected)) {
        ...
        if (expected < 0) {
          ...
          message =
              "wrong indentation: expected %d but found %d".formatted(expected, foundIndentation);
        }
        problems.add(
            new LintProblem(token.getStartMark().getLine() + 1, foundIndentation + 1, message));
      }
    }

    if (token instanceof org.yaml.snakeyaml.tokens.ScalarToken
        && (boolean) conf.get(OPTION_CHECK_MULTI_LINE_STRINGS)) {
    ...
    if (token instanceof BlockMappingStartToken) {
      ...
      //       a
      //     : 1
      if (!(next instanceof KeyToken)
          || next.getStartMark().getLine() != token.getStartMark().getLine()) {
        throw new UnexpectedTokenException();
      }
      ...
      //   - - a
      //     - b
      if (!(next instanceof BlockEntryToken)
          || next.getStartMark().getLine() != token.getStartMark().getLine()) {
        throw new UnexpectedTokenException();
      }
      ...
      if (!(next instanceof BlockEndToken
          || next instanceof FlowMappingEndToken
          || next instanceof FlowSequenceEndToken
          || next instanceof KeyToken)) {
        if (stack.get(stack.size() - 1).explicitKey) {
          //   ? k
          //   : value
          // or
          ...
          //      '- lib:\n'
          //      '  - var\n'
          if (context.get(OPTION_INDENT_SEQUENCES) instanceof Boolean
              && !(boolean) context.get(OPTION_INDENT_SEQUENCES)) {
            indent = stack.get(stack.size() - 1).indent;
          } else if (context.get(OPTION_INDENT_SEQUENCES) instanceof Boolean
              && (boolean) context.get(OPTION_INDENT_SEQUENCES)) {
            if (OPTION_CONSISTENT.equals(context.get(OPTION_SPACES))
                && next.getStartMark().getColumn() - stack.get(stack.size() - 1).indent == 0) {
    ...
    while (true) {
      if ((stack.get(stack.size() - 1).type == LABEL.F_SEQ
          ...
          || (stack.get(stack.size() - 1).type == LABEL.F_MAP
              ...
              && !consumedCurrentToken)
          || ((stack.get(stack.size() - 1).type == LABEL.B_MAP
                  || stack.get(stack.size() - 1).type == LABEL.B_SEQ)
              && token instanceof BlockEndToken
