 ...
 *
 * <ul>
 *   <li>{@code max-spaces-after} defines the maximal number of spaces allowed after hyphens. Set to
 *       a negative integer if you want to allow any number of spaces. Cannot be set to 0.
 *   <li>{@code min-spaces-after} defines the minimal number of spaces expected after hyphens. Set
 *       to a negative integer if you want to allow any number of spaces. When set to a positive
 *       value, cannot be greater than {@code max-spaces-after}.
 *   <li>YAMLLint will consider "-xx" as a scalar. However you may consider that, in your context,
 *       such a syntax is a typo and is actually a sequence and as a consequence there should be a
 *       space after the hyphen. As this is not a standard behaviour, you explicitly need to enable
 *       this control by setting the option {@code check-scalars} to {@code true}. <strong>Use with
 *       caution</strong> as all scalars will be checked and non-solvable false positive might be
 *       identified. Has no effect when set to {@code true} but {@code min-spaces-after} is disabled
 *       (&lt; 0).
 * </ul>
 *
 * <p>Default values (when enabled):
 *
 * <pre>
 *     rules:
 *       hyphens:
 ...
 * <p>Examples:
 *
 * <p>With <code>hyphens: {max-spaces-after: 1}</code> the following code snippet would **PASS**:
 *
 * <pre>
 *     - first list:
 *         - a
 ...
 * </pre>
 *
 * <p>With <code>hyphens: {max-spaces-after: 3}</code> the following code snippet would **PASS**:
 *
 * <pre>
 *     -   key
 *     -  key2
 ...
 * </pre>
 *
 * <p>With <code>hyphens: {min-spaces-after: 3}</code> the following code snippet would **PASS**:
 *
 * <pre>
 *     list:
 *     -   key
 ...
 * </pre>
 *
 * <p>With <code>hyphens: {min-spaces-after: 3, check-scalars: true}</code> the following code
 * snippet would **PASS**:
 *
 * <pre>
 *     list:
...
public class Hyphens extends TokenRule {
  ...
  public String validate(Map<String, Object> conf) {
    ...
    if (minSpaces > 0 && minSpaces > maxSpaces) {
      return "\"%s\" cannot be greater than \"%s\""
          .formatted(OPTION_MIN_SPACES_AFTER, OPTION_MAX_SPACES_AFTER);
    }
    return null;
  }
  ...
  public List<LintProblem> check(
    ...
    if (Boolean.TRUE.equals(conf.get(OPTION_CHECK_SCALARS))
        && (Integer) conf.get(OPTION_MIN_SPACES_AFTER) > 0
        && token instanceof ScalarToken scalarToken) {
