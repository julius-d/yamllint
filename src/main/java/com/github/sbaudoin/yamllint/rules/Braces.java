 ...
 *
 * <ul>
 *   <li>{@code forbid} is used to forbid the use of flow mappings which are denoted by surrounding
 *       braces (<code>{</code> and <code>}</code>). Use {@code true} to forbid the use of flow
 *       mappings completely. Use {@code non-empty} to forbid the use of all flow mappings except
 *       for empty ones.
 *   <li>{@code min-spaces-inside} defines the minimal number of spaces required inside braces.
 *   <li>{@code max-spaces-inside} defines the maximal number of spaces allowed inside braces.
 ...
 * </pre>
 *
 * <p>With <code>braces: {min-spaces-inside: 0, max-spaces-inside: 0}</code> the following code
 * snippet would **PASS**:
 *
 * <pre>object: {key1: 4, key2: 8}</pre>
 *
 ...
 * <pre>object: { key1: 4, key2: 8 }</pre>
 *
 * <p>With <code>braces: {min-spaces-inside: 1, max-spaces-inside: 3}</code> the following code
 * snippet would **PASS**:
 *
 * <pre>object: { key1: 4, key2: 8 }</pre>
 *
 ...
 * <pre>object: {key1: 4, key2: 8 }</pre>
 *
 * <p>With <code>braces: {min-spaces-inside-empty: 0, max-spaces-inside-empty: 0}</code> the
 ...
 * <pre>object: { }</pre>
 *
 * <p>With <code>braces: {min-spaces-inside-empty: 1, max-spaces-inside-empty: -1}</code> the
 * following code snippet would **PASS**:
 *
 * <pre>object: {         }</pre>
 *
  ...
  public List<LintProblem> check(
    ...
    if ((Boolean.TRUE.equals(conf.get(OPTION_FORBID)) && token instanceof FlowMappingStartToken)
        || ("non-empty".equals(conf.get(OPTION_FORBID))
            && token instanceof FlowMappingStartToken
    ...
    if (token instanceof FlowMappingStartToken && next instanceof FlowMappingEndToken) {
      problem =
          spacesAfter(
              ...
              next,
              (int)
                  ((((int) conf.get(OPTION_MIN_SPACES_INSIDE_EMPTY)) != -1)
                      ? conf.get(OPTION_MIN_SPACES_INSIDE_EMPTY)
                      : conf.get(OPTION_MIN_SPACES_INSIDE)),
              (int)
                  ((((int) conf.get(OPTION_MAX_SPACES_INSIDE_EMPTY)) != -1)
