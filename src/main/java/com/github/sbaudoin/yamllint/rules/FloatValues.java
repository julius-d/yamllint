...
/**
 * Use this rule to limit the permitted values for floating-point numbers. YAML permits three
 * classes of float expressions: approximation to real numbers, positive and negative infinity and
 * "not a number".
 *
 * <p>Options:
 *
 * <ul>
 *   <li>Use {@code require-numeral-before-decimal} to require floats to start with a numeral (eg.
 *       {@code 0.0} instead of {@code .0}).
 *   <li>Use {@code forbid-scientific-notation} to forbid scientific notation.
 *   <li>Use {@code forbid-nan} to forbid NaN (not a number) values.
 *   <li>Use {@code forbid-inf} to forbid infinite values.
 * </ul>
 *
 ...
 * <p>Examples:
 *
 * <p>With <code>float-values: {require-numeral-before-decimal: true}</code> the following code
 * snippets would **PASS**:
 *
 * <pre>
 *     anemometer:
 ...
 * </pre>
 *
 * <p>With <code>float-values: {forbid-scientific-notation: true}</code> the following code snippets
 * would **PASS**:
 *
 * <pre>
 *     anemometer:
 ...
 * </pre>
 *
 * <p>With <code>float-values: {forbid-nan: true}</code> the following code snippets would **FAIL**:
 *
 * <pre>
 *     anemometer:
 *       angle: .NaN
 * </pre>
 *
 * <p>With <code>float-values: {forbid-inf: true}</code> the following code snippets would **FAIL**:
 *
 * <pre>
 *     anemometer:
 *       angle: .inf
...
public class FloatValues extends TokenRule {
  ...
  public static final String OPTION_REQUIRE_NUMERAL_BEFORE_DECIMAL =
  ...
  public List<LintProblem> check(
    ...
    if ((boolean) conf.get(OPTION_FORBID_SCIENTIFIC_NOTATION)
        && val.matches("[-+]?(\\.\\d+|\\d+(\\.\\d*)?)([eE][-+]?\\d+)$")) {
      problems.add(
          new LintProblem(
              token.getStartMark().getLine() + 1,
              token.getStartMark().getColumn() + 1,
    ...
    if ((boolean) conf.get(OPTION_REQUIRE_NUMERAL_BEFORE_DECIMAL)
        && val.matches("[-+]?(\\.\\d+)([eE][-+]?\\d+)?$")) {
      problems.add(
          new LintProblem(
              token.getStartMark().getLine() + 1,
