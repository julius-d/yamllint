...
/**
 * The class represents a coding rule. It is not intended to be extended directly; instead, you are
 * encouraged to extend the derivated classes {@link CommentRule}, {@link LineRule} and {@link
 * TokenRule}.
 *
 * @see CommentRule
 * @see LineRule
...
public abstract class Rule {
  ...
  /**
   * Returns the ID of this rule. The default implementation returns the simple class' name (name
   * without package) in lower case and the "words" (identified by the capital letters) separated
   * with a hyphen.
   *
   * @return the rule ID
   */
  ...
  /**
   * Returns the rule's options. This is a very generic container: see the rule details for the
   * exact list and possible values for the options.
   *
   ...
   * @return an explanation for why the configuration is not valid, or {@code null} if the
  ...
  /**
   * Determines the number of spaces between the passed token and the next one and compares this
   * number with the min and max accepted values. In the event the determined number of spaces is
   * less than or greater than the min and max value, a problem is returned with the corresponding
   * error message.
   *
   * @param token the token to analyse
   * @param next the next token with which the spaces are counted
   * @param min the minimum number or expected spaces. May be <code>null</code>, in which case -1 is
   *     taken (no minimum required).
   * @param max the maximum number or expected spaces. May be <code>null</code>, in which case -1 is
   *     taken (no maximum required).
   * @param minDesc the problem description in case the number of spaces is less than <var>min</var>
   * @param maxDesc the problem description in case the number of spaces is greater than
   *     <var>max</var>
   * @return a problem or <code>null</code> if no problem found
   */
  protected LintProblem spacesAfter(
      Token token, Token next, Integer min, Integer max, String minDesc, String maxDesc) {
    ...
    if (next != null && token.getEndMark().getLine() == next.getStartMark().getLine()) {
      ...
      if (myMax != -1 && spaces > myMax) {
        return new LintProblem(
            token.getStartMark().getLine() + 1, next.getStartMark().getColumn(), maxDesc);
      } else if (myMin != -1 && spaces < myMin) {
        return new LintProblem(
            token.getStartMark().getLine() + 1, next.getStartMark().getColumn() + 1, minDesc);
      }
    }
    return null;
  }
  ...
  /**
   * Determines the number of spaces between the passed token and the previous one and compares this
   * number with the min and max accepted values. In the event the determined number of spaces is
   * less than or greater than the min and max value, a problem is returned with the corresponding
   * error message.
   *
   ...
   * @return a problem or <code>null</code> if no problem found
   */
  protected LintProblem spacesBefore(
      Token token, Token prev, Integer min, Integer max, String minDesc, String maxDesc) {
    int myMin = (min == null) ? -1 : min;
    int myMax = (max == null) ? -1 : max;

    if (prev != null
        && prev.getEndMark().getLine() == token.getStartMark().getLine()
        &&
      ...
      if (myMax != -1 && spaces > myMax) {
        return new LintProblem(
            token.getStartMark().getLine() + 1, token.getStartMark().getColumn(), maxDesc);
      } else if (myMin != -1 && spaces < myMin) {
        return new LintProblem(
            token.getStartMark().getLine() + 1, token.getStartMark().getColumn() + 1, minDesc);
      }
    }
    return null;
  }
   ...
   * </pre>
   *
   * A key is guessed to be explicit if it starts with a question mark. It is the responsibility of
   * the caller to check that the token corresponds to a key (the instance is a {@link
  ...
  protected boolean isExplicitKey(Token token) {
    return (token.getStartMark().getPointer() < token.getEndMark().getPointer()
        && token.getStartMark().getBuffer()[token.getStartMark().getPointer()] == '?');
  }
  ...
  protected int getLineIndent(Token token) {
    int start =
        rfind(token.getStartMark().getBuffer(), '\n', 0, token.getStartMark().getPointer()) + 1;
    int content = start;
  ...
  /**
   * The whitespace characters are: tab (ASCII 9), line feed (ASCII 10), vertical tab (ASCII 11),
   * form feed (ASCII 12), carriage return (ASCII 13), space (ASCII 32)
   *
   * @param i an integer representing a character
   * @return <code>true</code> if <var>i</var> is one of the characters listed above, <code>false
   *     </code> otherwise
   */
  ...
  /**
   * Returns the first index of an int in an array of int's. The int is searched for between the
   * start and end-1 indexes. If the int is not found, the method returns -1.
   *
   ...
   * @throws ArrayIndexOutOfBoundsException if <var>start</var> or <var>end</var> are beyond the
   *     boundaries of <var>haystack</var>
   */
  ...
  /**
   * Returns the right-most index of an int in an array of int's. The int is searched for between
   * the start and end-1 indexes. If the int is not found, the method returns -1.
   *
   * @param haystack the array of int's in which to search
   * @param needle the int to look for
   * @param start the index from which to search
  ...
  /**
   * Finds the line on which the token really ends. With SnakeYAML, scalar tokens often end on a
   * next line.
   *
   * @param token a token
  ...
  protected int getRealEndLine(Token token) {
    ...
    while (pos >= token.getStartMark().getPointer() - 1
        && isWhitespace(token.getEndMark().getBuffer()[pos])) {
  ...
  /**
   * Declares an option with the given name and default value. The type of the (default) value will
   * be used to check the rule configuration. If you pass a list, the first item will be used as the
   * default value. If this does not suits your need, use {@link #registerOption(String, Object,
   * Object)}, {@link #registerListOption(String, List)} or {@link #registerListOption(String, List,
   * List)} instead.
   *
  ...
  protected void registerOption(String name, Object value) {
    if (value instanceof List<?> list) {
      if (!list.isEmpty()) {
        registerOption(name, value, list.get(0));
      } else {
        throw new IllegalArgumentException(
            "Empty list passed, you must explicitly specify a default value");
      }
    } else {
  ...
  /**
   * Declares an option with the given name, value and default value. This method is mainly used
   * when the other method {@link #registerOption(String, Object)} is not sufficient, i.e. when the
   * option type cannot be the same as the default value (e.g. the values of the option are taken
   * from a list).
   *
   * @param name the option name
  ...
  /**
   * Declares an option as a list with the given name and default value. The type of the actual
   * configuration will be checked and is expected to be a list. The default value for this option
   * will also be the passed list. If you want to pass another default value, use {@link
   * #registerListOption(String, List, List)} instead.
   *
   * @param name the option name
   * @param value the default value
  ...
  /**
   * Declares an option as a list with the given name, value and default value. The type of the
   * actual configuration will be checked and is expected to be a list.
   *
   * @param name the option name
   * @param type the option's type
   * @param defaultValue the default value of the option
