 ...
 *
 * <ul>
 *   <li>Use {@code forbid-in-block-mappings} to prevent empty values in block mappings.
 *   <li>Use {@code forbid-in-flow-mappings} to prevent empty values in flow mappings.
 *   <li>Use {@code forbid-in-block-sequences} to prevent empty values in block sequences.
 * </ul>
 *
 ...
 * <p>Examples:
 *
 * <p>With <code>empty-values: {forbid-in-block-mappings: true}</code> the following code snippets
 * would **PASS**:
 *
 * <pre>
 *     some-mapping:
 ...
 * </pre>
 *
 * <p>With <code>empty-values: {forbid-in-flow-mappings: true}</code> the following code snippet
 * would **PASS**:
 *
 * <pre>
 *     {prop: null}
 ...
 * <pre> {a: 1, b:, c: 3}</pre>
 *
 * <p>With <code>empty-values: {forbid-in-block-sequences: true}</code> the following code snippet
 * would **PASS**:
 *
 * <pre>
 *     some-sequence:
  ...
  public List<LintProblem> check(
    ...
    if ((boolean) conf.get(OPTION_FORBID_IN_BLOCK_MAPPINGS)) {
      if (token instanceof ValueToken
          && (next instanceof KeyToken || next instanceof BlockEndToken)) {
        problems.add(
            new LintProblem(
    ...
    if ((boolean) conf.get(OPTION_FORBID_IN_FLOW_MAPPINGS)) {
      if (token instanceof ValueToken
          && (next instanceof FlowEntryToken || next instanceof FlowMappingEndToken)) {
        problems.add(
            new LintProblem(
                token.getStartMark().getLine() + 1,
                token.getEndMark().getColumn() + 1,
