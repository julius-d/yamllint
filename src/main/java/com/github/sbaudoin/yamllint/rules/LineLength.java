 ...
 *
 * <ul>
 *   <li>{@code max} defines the maximal (inclusive) length of lines.
 *   <li>{@code allow-non-breakable-words} is used to allow non breakable words (without spaces
 *       inside) to overflow the limit. This is useful for long URLs, for instance. Use {@code true}
 *       to allow, {@code false} to forbid.
 *   <li>{@code allow-non-breakable-inline-mappings} implies {@code allow-non-breakable-words} and
 *       extends it to also allow non-breakable words in inline mappings.
 * </ul>
 *
 * <p>Examples:
 *
 * <p>With <code>line-length: {max: 70}</code> the following code snippet would **PASS**:
 *
 * <pre>
 *     long sentence:
 ...
 * </pre>
 *
 * <p>With <code>line-length: {max: 60, allow-non-breakable-words: true}</code> the following code
 * snippet would **PASS**:
 *
 * <pre>
 *     this:
 ...
 * </pre>
 *
 * <p>With <code>line-length: {max: 60, allow-non-breakable-words: false}</code> the following code
 * snippet would **FAIL**:
 *
 * <pre>
 *     this:
...
public class LineLength extends LineRule {
  ...
  public static final String OPTION_ALLOW_NON_BREAKABLE_INLINE_MAPPINGS =
  ...
  public List<LintProblem> check(Map<Object, Object> conf, Parser.Line line) {
    ...
    if (line.getEnd() - line.getStart() > (int) conf.get("max")) {
      ...
      conf.put(
          OPTION_ALLOW_NON_BREAKABLE_WORDS,
          anbw || (boolean) conf.get(OPTION_ALLOW_NON_BREAKABLE_INLINE_MAPPINGS));
      if ((boolean) conf.get(OPTION_ALLOW_NON_BREAKABLE_WORDS)) {
        ...
        if (start != line.getEnd()) {
          ...
          if (start > line.getEnd()
              || line.getBuffer().substring(start, line.getEnd()).indexOf(' ') == -1) {
            return problems;
          }

          if ((boolean) conf.get(OPTION_ALLOW_NON_BREAKABLE_INLINE_MAPPINGS)
