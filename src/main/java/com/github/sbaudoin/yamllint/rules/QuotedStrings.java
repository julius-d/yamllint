...
/**
 * Use this rule to forbid any string values that are not quoted. You can also enforce the type of
 * the quote used.
 *
 * <p>Options:
 *
 * <ul>
 *   <li>{@code quote-type} defines allowed quotes: {@code single}, {@code double} or {@code any}
 *       (default).
 *   <li>{@code required} defines whether using quotes in string values is required ({@code true},
 *       default) or not ({@code false}), or only allowed when really needed ({@code
 *       "only-when-needed"}).
 *   <li>{@code extra-required} is a list of regexps to force string values to be quoted, if they
 *       match any regex. This option can only be used with {@code required: false} and {@code
 *       required: only-when-needed}.
 *   <li>{@code extra-allowed} is a list of regexps to allow quoted string values, even if {@code
 *       required: only-when-needed} is set.
 *   <li>{@code allow-quoted-quotes} allows ({@code true}) using disallowed quotes for strings with
 *       allowed quotes inside. Default {@code false}.
 * </ul>
 *
 * <p><strong>Note</strong>: Multi-line strings (with {@code |} or {@code >}) will not be checked.
 *
 * <p>Examples:
 *
 * <p>With <code>quoted-strings: {quote-type: any, required: true}</code> the following code snippet
 * would **PASS**:
 *
 * <pre>
 *     foo: "bar"
 ...
 * </pre>
 *
 * <p>With <code>quoted-strings: {quote-type: single, required: only-when-needed}</code> the
 * following code snippet would **PASS**:
 *
 * <pre>
 *     foo: bar
 *     bar: foo
 ...
 * </pre>
 *
 * <p>With <code>quoted-strings: {required: false, extra-required: [^http://, ^ftp://]}</code> the
 * following code snippet would **PASS**:
 *
 * <pre>
 *     - localhost
 ...
 *
 * <p>With <code>
 * quoted-strings: {required: only-when-needed, extra-allowed: [^http://, ^ftp://], extra-required: [QUOTED]}
 * </code> the following code snippet would **PASS**:
 *
 * <pre>
 *     - localhost
 *     - "http://localhost"
 ...
 * </pre>
 *
 * <p>With <code>quoted-strings: {quote-type: double, allow-quoted-quotes: false}</code> the
 * following code snippet would **PASS**:
 *
 * <pre>
 *     foo: "bar\\"baz"
 ...
 * </pre>
 *
 * <p>With <code>quoted-strings: {quote-type: double, allow-quoted-quotes: true}</code> the
 * following code snippet would **PASS**:
 *
 * <pre>
 *     foo: 'bar"baz'
...
public class QuotedStrings extends TokenRule {
  ...
  private static final String MSG_NOT_QUOTED_WITH_QUOTES =
  ...
  private static final String OCTAL_INT_RE =
      "^([-+]?0b[0-1_]+|[-+]?0o?[0-7_]+|[-+]?0[0-7_]+|[-+]?(?:0|[1-9][0-9_]*)|[-+]?0x[0-9a-fA-F_]+|[-+]?[1-9][0-9_]*(?::[0-5]?\\d)+)$";
  ...
  public QuotedStrings() {
    registerOption(
        OPTION_QUOTE_TYPE, Arrays.asList(QUOTE_STYLE_ANY, QUOTE_STYLE_SINGLE, QUOTE_STYLE_DOUBLE));
    registerOption(
        OPTION_REQUIRED, Arrays.asList(Boolean.class, String.class, ONLY_WHEN_NEEDED), true);
  ...
  public List<LintProblem> check(
    ...
    if (!(token instanceof ScalarToken
        && (prev instanceof BlockEntryToken
            || prev instanceof FlowEntryToken
            || prev instanceof FlowSequenceStartToken
            || prev instanceof TagToken
    ...
    if (conf.get(OPTION_REQUIRED) instanceof Boolean
        && Boolean.TRUE.equals(conf.get(OPTION_REQUIRED))) {
    ...
    } else if (conf.get(OPTION_REQUIRED) instanceof Boolean
        && Boolean.FALSE.equals(conf.get(OPTION_REQUIRED))) {
      ...
      if (((ScalarToken) token).getStyle() != DumperOptions.ScalarStyle.PLAIN
        ...
        boolean isExtraRequired =
            ((List<String>) conf.get(OPTION_EXTRA_REQUIRED))
                .stream()
                    .anyMatch(
      ...
      if (((ScalarToken) token).getStyle() != DumperOptions.ScalarStyle.PLAIN
          && tag == Tag.STR
          && ((ScalarToken) token).getValue() != null
          && !quotesAreNeeded(
              ((ScalarToken) token).getValue(), (int) context.get(FLOW_NEST_COUNT_KEY) > 0)) {
        boolean isExtraRequired =
            ((List<String>) conf.get(OPTION_EXTRA_REQUIRED))
                .stream()
                    .anyMatch(
        ...
        boolean isExtraAllowed =
            ((List<String>) conf.get(OPTION_EXTRA_ALLOWED))
                .stream()
                    .anyMatch(
                        r -> Pattern.compile(r).matcher(((ScalarToken) token).getValue()).find());
      ...
      // But when used need to match config
        ...
        boolean isExtraRequired =
            !((List<?>) conf.get(OPTION_EXTRA_REQUIRED)).isEmpty()
                && ((List<String>) conf.get(OPTION_EXTRA_REQUIRED))
                    .stream()
                        .anyMatch(
                            r ->
                                Pattern.compile(r)
                                    .matcher(((ScalarToken) token).getValue())
                                    .find());
        if (isExtraRequired) {
          msg = MSG_NOT_QUOTED;
        }
    ...
    if (msg != null) {
      problems.add(
          new LintProblem(
              token.getStartMark().getLine() + 1, token.getStartMark().getColumn() + 1, msg));
    }
  ...
  private boolean quoteMatch(String quoteType, DumperOptions.ScalarStyle tokenStyle) {
    return (QUOTE_STYLE_ANY.equals(quoteType)
        || (QUOTE_STYLE_SINGLE.equals(quoteType)
            && tokenStyle == DumperOptions.ScalarStyle.SINGLE_QUOTED)
        || (QUOTE_STYLE_DOUBLE.equals(quoteType)
            && tokenStyle == DumperOptions.ScalarStyle.DOUBLE_QUOTED));
  }
  ...
  private boolean quotesAreNeeded(String string, boolean isInsideAFlow) {
    ...
    // Remove the 5 first tokens corresponding to 'key: ' (StreamStartToken, BlockMappingStartToken,
    // KeyToken, ScalarToken(value = key), ValueToken)
    for (int i = 0; i < 5; i++) {
      loader.getToken();
    }
    try {
      ...
      return !(a instanceof ScalarToken st
          && st.getStyle() == DumperOptions.ScalarStyle.PLAIN
          && b instanceof BlockEndToken
          && string.equals(st.getValue()));
  ...
  private boolean hasQuotedQuotes(Token token) {
    return (token instanceof ScalarToken st
        && ((st.getStyle() == DumperOptions.ScalarStyle.SINGLE_QUOTED
                && st.getValue().contains("\""))
            || (st.getStyle() == DumperOptions.ScalarStyle.DOUBLE_QUOTED
                && st.getValue().contains("'"))));
  }
}
