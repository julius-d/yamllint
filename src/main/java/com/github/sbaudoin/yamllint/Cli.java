...
public final class Cli {
  ...
  public static final Map<String, OutputFormat> OUTPUT_FORMATS =
  ...
  /**
   * Name of a local rule configuration file: if this file is present in the work directory, it is
   * used as an extension configuration file
   */
  ...
  public void run(final String[] args) {
    ...
    for (String path : findFilesRecursively(conf, (String[]) arguments.get(ARG_FILES_OR_DIR))) {
      try (InputStream in = "-".equals(path) ? System.in : new FileInputStream(path)) {
        ...
        List<LintProblem> problems =
            Linter.run(in, conf, file).stream()
                .filter(
                    problem ->
                        Boolean.FALSE.equals(arguments.get(ARG_NO_WARNINGS))
                            || problem.getLevel() == null
                            || Linter.ERROR_LEVEL.equals(problem.getLevel()))
                .collect(Collectors.toList());
        String output =
            Format.format(file.getPath(), problems, OUTPUT_FORMATS.get(arguments.get(ARG_FORMAT)));
        if (!"".equals(output)) {
          out(output);
        }
        // Save max level
        int level =
            problems.stream()
                .mapToInt(problem -> (Integer) Linter.getProblemLevel(problem.getLevel()))
                .max()
                .orElse(0);
        if (level > maxLevel) {
          maxLevel = level;
        ...
        err(
            "Cannot read "
                + ("-".equals(path) ? "standard input" : ("file `" + path + "'"))
                + ", skipping");
      }

      // Add an extra line break for standard and colored formats
      if (!first
          && OUTPUT_FORMATS.get(arguments.get(ARG_FORMAT)) != OutputFormat.PARSABLE
          && OUTPUT_FORMATS.get(arguments.get(ARG_FORMAT)) != OutputFormat.GITHUB) {
        out("");
      }
      first = false;
    }
    ...
    if (maxLevel == (int) Linter.getProblemLevel(Linter.ERROR_LEVEL)) {
      System.exit(1);
        ...
        && Boolean.TRUE.equals(arguments.get(ARG_STRICT))) {
      System.exit(2);
    }
  ...
  private Options prepareOptions() {
    ...
    og.addOption(
        Option.builder("c")
            .longOpt("config-file")
            .hasArg()
            .argName(ARG_CONFIG_FILE)
            .desc("path to a custom configuration")
            .build());
    og.addOption(
        Option.builder("d")
            .longOpt("config-data")
            .hasArg()
            .argName(ARG_CONFIG_DATA)
            .desc("custom configuration (as YAML source)")
            .build());
        ...
        Option.builder("f")
            ...
            .desc(
                "format for parsing output: "
                    + OUTPUT_FORMATS.keySet().stream()
                        .map(
                            f ->
                                (DEFAULT_FORMAT.equals(f))
                                    ? ("'" + f + "' (default)")
                                    : ("'" + f + "'"))
    ...
    options.addOption(
        Option.builder()
            .longOpt(ARG_NO_WARNINGS)
            .hasArg(false)
            .argName(ARG_NO_WARNINGS)
            .desc("output only error level problems")
            .build());
    options.addOption(
        Option.builder()
            .longOpt(ARG_LIST_FILES)
            .hasArg(false)
            .argName(ARG_LIST_FILES)
            .desc("list files to lint and exit")
            .build());
    options.addOption(
        Option.builder("s")
            .longOpt(ARG_STRICT)
            .hasArg(false)
            .argName(ARG_STRICT)
            .desc("return non-zero exit code on warnings as well as errors")
            .build());
  ...
  private CommandLine parseCommandLine(final Options options, final String[] args) {
    ...
    try {
      ...
      if (!OUTPUT_FORMATS.containsKey(format)) {
        endOnError(
            "invalid output format '%1$s'. Supported formats: %2$s"
                .formatted(
                    format,
                    OUTPUT_FORMATS.keySet().stream()
                        .map(
  ...
  private YamlLintConfig getYamlLintConfig(final Map<String, Object> arguments) {
    ...
    try {
      ...
      if (arguments.containsKey(ARG_CONFIG_DATA) && arguments.get(ARG_CONFIG_DATA) != null) {
        if (!"".equals(arguments.get(ARG_CONFIG_DATA))
            && !((String) arguments.get(ARG_CONFIG_DATA)).contains(":")) {
        ...
        return new YamlLintConfig(
            new File((String) arguments.get(ARG_CONFIG_FILE)).toURI().toURL());
  ...
  /**
   * Returns the path to the user's yamllint global configuration file, as per the environment
   * setting
   *
  ...
  /**
   * Returns the {@code File}, possibly {@code null}, that points to the project configuration file.
   * This file is identified in the following order:
   *
   * <ol>
   *   <li>{@code .yamllint} file in the project's directory, i.e. the current working directory
   *   <li>{@code .yamllint.yaml} file in the project's directory, i.e. the current working
   *       directory
   *   <li>{@code .yamllint.yml} file in the project's directory, i.e. the current working directory
   * </ol>
   *
   ...
   *
   * <ol>
   *   <li>{@code .yamllint} file
   *   <li>{@code .yamllint.yaml} file
   *   <li>{@code .yamllint.yml} file
   * </ol>
   *
   * If the file cannot be found in the passed directory, the method looks for it in the parent
   * directory unless it has reached the user's home directory or a system root directory.
   *
  ...
  /**
   * Processes recursively the passed paths to build and return a list of expected YAML files (file
        ...
        files.addAll(
            findFilesRecursively(
                ...
                Arrays.stream(file.list())
                    .map(name -> file.getPath() + File.separator + name)
                    .collect(Collectors.toList())
                    .toArray(new String[] {})));
