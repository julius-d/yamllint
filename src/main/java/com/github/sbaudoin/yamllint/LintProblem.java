...
public class LintProblem {
   ...
   * @param desc human-readable description of the problem (defaulted to {@code "<no description>"}
   *     if {@code null})
   */
   ...
   * @param line line on which the problem was found (starting at 1)
   * @param column column on which the problem was found (starting at 1)
   * @param desc hman-readable description of the problem (defaulted to {@code "<no description>"}
   *     if {@code null})
   * @param ruleId identifier of the rule that detected the problem. May be {@code null}.
   * @param extraDesc extra, additional description (i.e. may be {@code null}, in which case it is
   *     ignored)
   */
  ...
  /**
   * Returns a message representing this problem: the description followed by the rule Id between
   * parenthesis
   *
  ...
  public String getLongMessage() {
    if (ruleId != null) {
      return "%1$2s (%2$2s)%3$s%4$s"
          .formatted(
              desc,
              ruleId,
              (extraDesc == null) ? "" : System.lineSeparator(),
  ...
  /**
   * Returns the level of the problem, one of levels defined in {@link Linter} (or <code>null</code>
   * by default)
   *
